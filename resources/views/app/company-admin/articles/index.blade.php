...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-lg-3 col-md-6 col-sm-12">
            <x-adminlte-small-box title="0" text="Publicados" icon="fas fa-check-circle" theme="success"
                id="stat-published" />
        </div>
        <div class="col-lg-3 col-md-6 col-sm-12">
    ...
    <x-adminlte-modal id="modal-form" title="Nuevo Artículo" theme="primary" icon="fas fa-plus-circle" size="xl" scrollable>
        <form id="article-form">
            ...
            <x-adminlte-textarea id="form-content" name="content" label="Contenido" rows="10"
                placeholder="Escribe el contenido completo del artículo (soporta Markdown)..." enable-old-support>
                <x-slot name="prependSlot">
                    <div class="input-group-text">
                ...
                <x-slot name="bottomSlot">
                    <small class="form-text text-muted">
                        <i class="fab fa-markdown"></i> Soporta formato Markdown. Mínimo 50 caracteres requeridos.
                    </small>
                </x-slot>
            </x-adminlte-textarea>
...
@section('js')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // =====================================================================
            // TOAST NOTIFICATIONS (AdminLTE v3 Official - Same as Categories)
            // =====================================================================
            ...
            const Toast = {
                ...
                error(message, title = 'Error') {
                    $(document).Toasts('create', {
                        ...
                        body: message,
                        autohide: true,
                        delay: CONFIG.TOAST_DELAY + 3000, // Errors stay longer
                 ...
                 * @param {object} errors - Object with field names as keys and arrays of error messages as values
                 */
                validationErrors(errors, title = 'Error de Validación') {
                    let errorMessages = [];
                    for (const field in errors) {
                        if (errors.hasOwnProperty(field)) {
                            const fieldErrors = errors[field];
                            const translatedErrors = fieldErrors.map(err => translateValidationError(err));
                            errorMessages = errorMessages.concat(translatedErrors);
                        }
                    }
                    ...
                    $(document).Toasts('create', {
                        ...
                        body: message,
                        autohide: true,
                        delay: CONFIG.TOAST_DELAY + 5000, // Validation errors stay even longer
            ...
            /**
             * Translate common API validation error messages to Spanish
             */
            function translateValidationError(errorMessage) {
                const translations = {
                    'The category_id field is required.': 'La categoría es obligatoria.',
                    'The selected category_id is invalid.': 'La categoría seleccionada no es válida.',
                    'The title field is required.': 'El título es obligatorio.',
                    'The title must be at least 3 characters.': 'El título debe tener al menos 3 caracteres.',
                    'The title must not be greater than 255 characters.': 'El título no puede exceder los 255 caracteres.',
                    'The title has already been taken.': 'Ya existe un artículo con este título.',
                    'The content field is required.': 'El contenido es obligatorio.',
                    'The content must be at least 50 characters.': 'El contenido debe tener al menos 50 caracteres.',
                    'The excerpt must not be greater than 500 characters.': 'El resumen no puede exceder los 500 caracteres.',
                    'Article is already published': 'El artículo ya está publicado.',
                    'Article is not published': 'El artículo no está publicado.',
                    'No se puede eliminar un artículo publicado': 'No se puede eliminar un artículo publicado. Primero despublícalo.'
                };
            ...
            function populateCategorySelect() {
                ...
                categories.forEach(cat => {
                    ...
                    option.value = cat.id;
                    // Translate category name to Spanish
                    option.textContent = translateCategoryName(cat.name);
            ...
            function renderArticlesTable(articles) {
                ...
                tbody.innerHTML = articles.map(article => {
                    ...
                    const categoryObj = categories.find(c => c.id === article.category_id);
                    const categoryName = categoryObj ? translateCategoryName(categoryObj.name) : 'N/A';
                    const categoryCode = categoryObj?.code || '';
            ...
            function openViewModal(articleId) {
                ...
                const categoryObj = categories.find(c => c.id === currentArticle.category_id);
                const categoryName = categoryObj ? translateCategoryName(categoryObj.name) : 'N/A';
                const categoryCode = categoryObj?.code || '';
            ...
            function openFormModal(mode, articleId = null) {
                ...
                if (mode === 'create') {
                    ...
                    $('#article-form').find('.is-invalid').removeClass('is-invalid');

                    if (!categoriesLoaded) {
                        Toast.warning('Las categorías aún están cargando, espera un momento.');
                    }
                } else if (mode === 'edit') {
                    currentArticle = allArticles.find(a => a.id === articleId);
                    if (!currentArticle) {
            ...
            function saveArticle() {
                ...
                if (!$form.valid()) {
                    console.log('[Articles] Form validation failed');
                    return;
                }
                ...
                fetch(url, {
                    ...
                    .then(({ status, data }) => {
                        if (status >= 200 && status < 300 && (data.data || data.success)) {
                            Toast.success(isCreate ? 'Artículo creado exitosamente' : 'Artículo actualizado exitosamente');
                            $('#modal-form').modal('hide');
                            loadArticles(getCurrentFilters(), currentPage);
                        } else if (status === 422 && data.errors) {
                            // Handle validation errors from API
                            console.error('[Articles] Validation errors:', data.errors);
                            Toast.validationErrors(data.errors);
                        } else if (status === 403) {
                            Toast.error(translateValidationError(data.message) || 'No tienes permiso para realizar esta acción.');
                        } else if (status === 401) {
                            Toast.error('Tu sesión ha expirado. Por favor, inicia sesión nuevamente.');
                            setTimeout(() => window.location.href = '/login', 2000);
                        } else {
                            Toast.error(translateValidationError(data.message) || 'Error al guardar el artículo. Inténtalo nuevamente.');
                        }
                    })
                    .catch(error => {
                        console.error('Error saving article:', error);
                        Toast.error('Error de conexión. Verifica tu conexión a internet e inténtalo nuevamente.');
                    })
                    .finally(() => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        fetch(`${apiUrl}/help-center/articles/${articleId}/publish`, {
                            ...
                            .then(({ status, data }) => {
                                if (status >= 200 && status < 300 && (data.data || data.success)) {
                                    Toast.success('Artículo publicado exitosamente. Ahora es visible para los usuarios.');
                                    loadArticles(getCurrentFilters(), currentPage);
                                } else if (status === 400) {
                                    Toast.warning(translateValidationError(data.message) || 'El artículo ya está publicado.');
                                } else {
                                    Toast.error(translateValidationError(data.message) || 'Error al publicar el artículo.');
                                }
                            })
                            .catch(error => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        fetch(`${apiUrl}/help-center/articles/${articleId}/unpublish`, {
                            ...
                            .then(({ status, data }) => {
                                if (status >= 200 && status < 300 && (data.data || data.success)) {
                                    Toast.success('Artículo despublicado. Volvió a estado borrador.');
                                    loadArticles(getCurrentFilters(), currentPage);
                                } else if (status === 400) {
                                    Toast.warning(translateValidationError(data.message) || 'El artículo no está publicado.');
                                } else {
                                    Toast.error(translateValidationError(data.message) || 'Error al despublicar el artículo.');
                                }
                            })
                            .catch(error => {
            ...
            function openDeleteModal(articleId) {
                ...
                if (currentArticle.status !== 'DRAFT') {
                    Toast.error('Solo se pueden eliminar artículos en estado BORRADOR. Primero despublica el artículo.');
                    return;
                }
            ...
            function deleteArticle() {
                ...
                fetch(`${apiUrl}/help-center/articles/${articleId}`, {
                    ...
                    .then(({ status, data }) => {
                        if (status >= 200 && status < 300 && (data.success || data.data?.success)) {
                            Toast.success('Artículo eliminado permanentemente.');
                            $('#modal-delete').modal('hide');
                            loadArticles(getCurrentFilters(), currentPage);
                        } else if (status === 403) {
                            Toast.error(translateValidationError(data.message) || 'No se puede eliminar un artículo publicado.');
                        } else {
                            Toast.error(translateValidationError(data.message) || 'Error al eliminar el artículo.');
                        }
                    })
                    .catch(error => {
            ...
            // =====================================================================
            // JQUERY VALIDATION (AdminLTE v3 Official Pattern)
            // =====================================================================

            function initFormValidation() {
                console.log('[Articles] Checking jQuery Validation Plugin...');
                
                if (typeof $.fn.validate === 'undefined') {
                    console.error('[Articles] ERROR: jQuery Validation Plugin NOT loaded!');
                    console.log('[Articles] Form will rely on HTML5 validation and server-side validation');
                    return;
                }

                console.log('[Articles] ✓ jQuery Validation Plugin loaded correctly');
                ...
                $form.validate({
                    ...
                    messages: {
                        ...
                        excerpt: {
                            maxlength: 'El resumen no puede exceder los 500 caracteres'
                        },
                        content: {
                ...
                // FIX: Select elements need manual re-validation on change
                $('#form-category').on('change', function() {
                    $form.validate().element('#form-category');
                });

                console.log('[Articles] ✓ Form validation configured successfully');
            }
            ...
            // Legacy showAlert function - now wraps the official Toast
